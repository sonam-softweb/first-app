 ...
 */
(function(jQuery) {
	jQuery.fn.horizontalBarGraph = function(options) {
	
		var opts = jQuery.extend({}, jQuery.fn.horizontalBarGraph.defaults, options);
	...
	function createShowButton(opts, graph) {
		var button = jQuery("<span />").text(opts.button).addClass("hbg-show-button");
	...
	function createBar(e, opts) {
		...
		e.html(jQuery("<span/>").html(val).addClass("hbg-value"));
		applyOptions(e, opts);
	}
	...
	function tryShowTitle(graph) {
		...
		if(title) {
			jQuery("<div/>").text(title).addClass("hbg-title").insertBefore(graph);
	...
	function setColors(bars, opts) {
		...
		bars.each(function() { 
			...
			jQuery(this).css({backgroundColor: opts.colors[c]});
		});
	}
	...
	function setHoverColors(bars, opts) {
		...
		bars.each(function(i) {
			...
			bar.hover(function() {
				jQuery(this).css({backgroundColor: hc});
			}, function() {
				jQuery(this).css({backgroundColor: c});
			});
		});
	}
	...
	jQuery.fn.horizontalBarGraph.defaults = {
		interval: 1,
		hasTitles: false,
		animated: false,
		button: 'Show Values',
